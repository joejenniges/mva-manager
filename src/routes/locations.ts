...
router.post("/", requirePermission("edit", "locations"), validate(CreateLocationSchema), async (req, res, next) => {
  try {
    res.status(201).json(await service.createLocation(res.locals.eventId, req.body));
  } catch (err) { next(err); }
});

router.patch("/:id", requirePermission("edit", "locations"), validate(UuidParam, "params"), validate(UpdateLocationSchema), async (req, res, next) => {
  try {
    res.json(await service.updateLocation(res.locals.eventId, res.locals.params.id, req.body));
  } catch (err) { next(err); }
});

router.delete("/:id", requirePermission("delete", "locations"), validate(UuidParam, "params"), async (req, res, next) => {
  try {
