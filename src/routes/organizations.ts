...
router.post("/", requirePermission("edit", "organizations"), validate(CreateOrganizationSchema), async (req, res, next) => {
  try {
    res.status(201).json(await service.createOrganization(res.locals.eventId, req.body));
  } catch (err) { next(err); }
});

router.patch("/:id", requirePermission("edit", "organizations"), validate(UuidParam, "params"), validate(UpdateOrganizationSchema), async (req, res, next) => {
  try {
    res.json(await service.updateOrganization(res.locals.eventId, res.locals.params.id, req.body));
  } catch (err) { next(err); }
});

router.delete("/:id", requirePermission("delete", "organizations"), validate(UuidParam, "params"), async (req, res, next) => {
  try {
