...
router.post("/", validate(CreateAppointmentTemplateSchema), async (req, res, next) => {
  try {
...
router.patch("/:id", validate(UuidParam, "params"), validate(UpdateAppointmentTemplateSchema), async (req, res, next) => {
  try {
